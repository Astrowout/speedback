...
const NewProjectForm: FunctionComponent<NewProjectFormProps> = ({ className, edit = false }) => {
	...
	const [createProject, { loading: isLoading, error }] = useMutation(edit ? Mutations.updateProject : Mutations.createProject);
	...
	const onSubmit = async (data: FormValues) => {
		const { data: { createProject: project } } = await createProject({ variables: { issuer: user.issuer, ...data } });
	...
	return (
		<form className={cn(className, "space-y-12")} onSubmit={handleSubmit(onSubmit)}>
